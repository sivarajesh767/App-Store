...
class AppStore extends Component {
  ...
  setActiveTabId = tabId => {
    this.setState({searchInput: tabId})
  }
  ...
  getFilteredId = searchedApps => {
    const {searchInput} = this.state
    const filteredId = searchedApps.filter(
      each => each.category === searchInput,
    )
    return filteredId
  }
  ...
  render() {
    ...
    const filteredId = this.getFilteredId(searchResult)
    return (
      <div className="Bg-Co">
        <div className="appStore-Co">
          ...
          <div>
            ...
            <ul className="unord-2">
              {filteredId.map(each => (
                <AppItem key={each.appId} eachappDetails={each} />
              ))}
            </ul>
          </div>
        </div>
